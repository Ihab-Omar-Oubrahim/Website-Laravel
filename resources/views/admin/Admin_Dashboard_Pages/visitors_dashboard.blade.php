...
<link rel="stylesheet" href="{{ asset('assets/CSS/Admin/Dashboard_Rows_CSS/userVisits_rows.css') }}">
{{-- for sharedMsg page --}}
...
@auth
        ...
        <div class="main-content">
            <div class="dashboard_rows_container" id="dashboard_rows_container">
                <div class="dashboard_rows_header">
                    ...
                    <div class="dashboard_header_left">
                        <div class="dashboard_logo_container">
                            <img src="{{ asset('assets/res/Admin_Res/sidebar_res/view.png') }}" alt="dashboard_logo"
                                class="logo" />
                        </div>
                        <div class="dashboard_header_info">
                            <h1 class="header_title">Visitors view </h1>
                            <p class="header_description">View all website visitors</p>
                        </div>
                    </div>

                    <!-- Right Section: Search Bar -->
                    <div class="dashboard_header_right">
                        <form method="GET" action="{{ route('visitors.search') }}"
                ...
                <div class="dashboard_rows_body" id="dashboardContainer">
                    ...
                    <div class="Visitors_Row_Container">
                        @forelse ($userVisits as $userVisit)
                            <div class="Visitor_Row">
                                <div class="visitor_id_checkbox">
                                    <input type="checkbox" class="edit_checkbox" id="checkbox_{{ $userVisit->id }}"
                                        data-id="{{ $userVisit->id }}" value="{{ $userVisit->id }}" />
                                    <label for="checkbox_{{ $userVisit->id }}" class="custom_checkbox"></label>
                                </div>
                                <div class="visitor_img">
                                    @if ($userVisit->user_id)
                                        <img src="{{ asset('assets/res/Admin_Res/sidebar_res/user.png') }}"
                                            alt="valid_user">
                                    @else
                                ...
                                <div class="visitor_info">
                                    <p id="visitor_name">
                                        {{ $userVisit->user_id ? $userVisit->user->name : 'Unknown' }}
                                    </p>
                                    <p id="visitor_id">
                                        {{ $userVisit->user_id ? $userVisit->user_id : 'Unknown' }}
                                    </p>
                                    <hr class="visitor_line">
                                    <p style="margin-bottom: 10px;"> Ip address : </p>
                                    <p id="visitor_ip"> {{ $userVisit->ip_address }} </p>
                                </div>
                            </div>
                        @empty
                            <div class="Empty_Visitors">
                                <p> No Visitors </p>
                            </div>
                        @endforelse
                    </div>
    ...
    @include('admin.Confirmations.Delete_Visitor_Confirmation')
    {{-- Delete Confirmation For Contact --}}
...
{{-- JavaScript Contact --}}
<script src="{{ asset('assets/JS/Admin/Visitors/visitors.js') }}"></script>
