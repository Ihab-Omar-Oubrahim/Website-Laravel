function viewSharedMessage(username, userID, comment_message, createdAt, comment_id) {
    ...
    document.querySelector('.user_Comment').innerHTML = comment_message || 'Empty Comment'; // innerHTML instead of textContent because
                                                                                            // we are passing <br> in the html button
    document.getElementById('popupCreatedAt').textContent = `Created At: ${createdAt || 'N/A'}`;
    document.getElementById('popupID').textContent = `Row ID - ${comment_id || 'N/A'}`;
